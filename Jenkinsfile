pipeline {
    ...
    stages {
        ...
        stage('create docker image') {
            steps {
                bat 'docker build -t i-arshdeepsingh070-feature:${BUILD_NUMBER} --no-cache -f Dockerfile .'
            }
        }
        stage('check and push') {
            parallel {
                ...
                stage('push to docker hub') {
                    steps {
                        bat 'docker tag i-arshdeepsingh070-feature:${BUILD_NUMBER} ${repository}:${BUILD_NUMBER}'
                        bat 'docker tag i-arshdeepsingh070-feature:${BUILD_NUMBER} ${repository}:latest'
